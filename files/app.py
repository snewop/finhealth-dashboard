...
def render_ai_assistant_tab(metrics_df: pd.DataFrame, company_info: dict) -> None:
    """Affiche le chat de l'Assistant IA (désactivé temporairement)."""
    st.markdown("### 💬 Assistant IA Financier")
    st.info("ℹ️ Le module d'Intelligence Artificielle basé sur Google Generative AI a été temporairement désactivé en raison de la dépréciation de son ancienne bibliothèque Python. Une mise à jour vers la nouvelle API `google-genai` sera nécessaire pour le réactiver.")
